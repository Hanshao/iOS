...
- (void)viewDidLoad {
    ...
    [self.analysisView addRefreshingBlock:^{
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            ...
            [self.analysisView endRefreshing];
            for (int i = 0; i < 5; ++ i) {
                [self.dataArray insertObject:@(arc4random()%150) atIndex:0];
                [self.curveArray insertObject:@(arc4random()%150) atIndex:0];
            }
            [self.analysisView reloadData];
        });
    }];
