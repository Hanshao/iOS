...
- (NSArray<UICollectionViewLayoutAttributes *> *)layoutAttributesForElementsInRect:(CGRect)rect {
    ...
    CGRect rectVisible = CGRectZero;
    rectVisible.origin = self.collectionView.contentOffset;
    rectVisible.size = self.collectionView.bounds.size;
    
    for (UICollectionViewLayoutAttributes *attribute in array) {
        UICollectionViewLayoutAttributes *copyAttr = [attribute copy];
        if (copyAttr.indexPath.row < [self.collectionView numberOfItemsInSection:0]) {
            if (!CGRectIntersectsRect(copyAttr.frame, rect)) continue;
            // 距离显示中心的距离
            CGFloat distance = CGRectGetMidY(rectVisible) - copyAttr.center.y;
            if (ABS(distance) > ACTIVE_DISTANCE) continue;
            // 归一化的距离
            CGFloat scale = (ACTIVE_DISTANCE - ABS(distance)) / ACTIVE_DISTANCE;
            // 设置属性
            copyAttr.transform = CGAffineTransformMakeScale(scale, scale);
            copyAttr.zIndex = 1;
            [resArray addObject:copyAttr];
        }
    }
...
- (void)layoutSubviews {
    ...
    for (int i = 0; i < numberOfComponents; ++ i, total += unit) {
        ...
        // 0.25的偏移量, 0.25即0.5像素, 系统自动舍入到1像素
