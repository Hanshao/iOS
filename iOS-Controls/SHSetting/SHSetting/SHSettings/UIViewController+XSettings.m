...
static void *xSettingsDataSource = (void *)@"xSettingsDataSource";
...
- (id<XTableViewDataSource>)xDataSource {
    return objc_getAssociatedObject(self, &xSettingsDataSource);
}
- (void)setXDataSource:(id<XTableViewDataSource>)xDataSource {
    objc_setAssociatedObject(self, &xSettingsDataSource, xDataSource, OBJC_ASSOCIATION_ASSIGN);
}

- (NSMutableArray *)xSettingGroups {
    NSMutableArray *groups = objc_getAssociatedObject(self, &xSettingsSettingGroups);
    if (!groups) {
        NSArray *source = [self.xDataSource settingItems];
