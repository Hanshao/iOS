...
@interface XScaner () <AVCaptureMetadataOutputObjectsDelegate>
...
@property (weak, nonatomic) id<XScanerDelegate>            delegate;           // 代理

@end

@implementation XScaner
...
- (instancetype)initWithDelegate:(id<XScanerDelegate>)delegate queue:(dispatch_queue_t)queue codeTypes:(NSArray *)types {
    if (self = [super init]) {
        ...
        AVAuthorizationStatus status = [[self class] authorizationStatusForVideoType];
...
- (void)setupQueue:(dispatch_queue_t)queue codeTypes:(NSArray *)codeTypes {
    ...
    outputConnection.videoOrientation = [XScaner videoOrientationFromCurrentDeviceOrientation];
}
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
    if ([metadataObjects count] > 0 && [self.delegate respondsToSelector:@selector(scaner:didCapture:)]) {
        AVMetadataMachineReadableCodeObject *metaCodeObject = [metadataObjects firstObject];
        dispatch_async(dispatch_get_main_queue(), ^{
            [self.delegate scaner:self didCapture:[metaCodeObject stringValue]];
        });
    }
}
