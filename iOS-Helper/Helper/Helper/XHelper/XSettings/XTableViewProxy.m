...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    XSettingModel *item = [self.settingsProxy.xSettingGroups[section] items][row];
    // 显示的cell
    XSettingViewCell *cell = nil;
    // 如果有自定义的cell类型
    if (item.cellType) {
        cell = [item.cellType settingCellWithTalbeView:tableView cellAttrs:self.settingsProxy.xCellAttrs];
    } else { // 使用默认类型
        cell = [XSettingViewCell settingCellWithTalbeView:tableView cellAttrs:self.settingsProxy.xCellAttrs];
    }
    // 绑定item
    cell.item = item;
