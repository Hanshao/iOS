...
+ (instancetype)packageWithPackage:(Package *)package {
    ...
    newPackage.style = package.style;
    newPackage.accessory = package.accessory;
...
- (instancetype)initWithData:(NSData *)data {
    return [self initWithData:data style:-1 accessory:nil];
}
- (instancetype)initWithData:(NSData *)data style:(NSInteger)style {
    return [self initWithData:data style:style accessory:nil];
}
- (instancetype)initWithData:(NSData *)data style:(NSInteger)style accessory:(id)accessory {
    if (!(self = [super init])) return self;
    if ((data.length) < sizeof(Header)) return self; // 数据部分起始偏移量
    
    _style = style; _accessory = accessory;
