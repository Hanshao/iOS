...
static const UInt8 CodeSubscribeOfConfig    = 0x65;         // 配置订阅码
...
+ (Package *)joinWorkServer:(UInt16)serial account:(NSString *)account key:(NSString *)key company:(UInt8)company
                       type:(UInt8)type author:(UInt16)author joinCode:(unsigned const char*)joinCode joinSize:(int)joinSize;
...
+ (Package *)onlineQuery:(UInt16)serial mac:(NSString *)mac company:(UInt8)company type:(UInt8)type author:(UInt16)author;
 ...
 * 参数 urlString 升级地址
 */
typedef void (^FirmwareVerParser)(NSString *mac, NSString *softVer, NSString *url);
+ (Package *)firmwareVersion:(UInt16)serial mac:(NSString *)mac company:(UInt8)company type:(UInt8)type author:(UInt16)author;
...
+ (Package *)subscribe:(UInt16)serial mac:(NSString *)mac company:(UInt8)company type:(UInt8)type author:(UInt16)author code:(UInt8)code enable:(BOOL)enable;
 ...
 * 参数 nickName 设备别名
 */
typedef void (^QueryParser)(NSString *mac, NSInteger style, NSString *hardVer, NSString *softVer, NSString *nickName);
 ...
 */
typedef void (^FinderParser)(NSString *mac, NSString *ip, UInt8 company, UInt8 type, UInt16 author, id obj);
...
+ (Package *)ulock:(UInt16)serial mac:(NSString *)mac company:(UInt8)company type:(UInt8)type author:(UInt16)author lock:(BOOL)lock;
