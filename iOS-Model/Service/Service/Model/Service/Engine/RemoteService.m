...
- (void)setSSLSettings:(NSDictionary *)sslSettings trustHandler:(TrustHandler)trustHandler {
    NSMutableDictionary *settings = [NSMutableDictionary dictionaryWithDictionary:sslSettings];
    [settings setObject:@YES forKey:GCDAsyncSocketManuallyEvaluateTrust];
    self.sslSettings = [NSDictionary dictionaryWithDictionary:settings];
...
- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag {
    // 数据包处理
    NSLog(@"remote %@ did read>>>>>>>>>%@", sock, [self hex:data]);
...
- (void)socket:(GCDAsyncSocket *)sock didWriteDataWithTag:(long)tag {
    NSLog(@"remote %@ did write>>>>>>>>>>", sock);
}
/**
 * Called when a socket disconnects with or without error.
 */
- (void)socketDidDisconnect:(GCDAsyncSocket *)sock withError:(NSError *)err {
    NSLog(@"remote %@ disconnect>>>>>>>>>%@", sock, err);
    if ([self.delegate respondsToSelector:@selector(remoteServiceDidDisconnect:error:)]) {
...
- (void)socket:(GCDAsyncSocket *)sock didReceiveTrust:(SecTrustRef)trust completionHandler:(void (^)(BOOL shouldTrustPeer))completionHandler {
    NSLog(@"remote %@ didRecieveTrust>>>>>>>>>", sock);
    // 证书验证,默认信任所有主机
...
- (void)sendPacket:(NSData *)data {
    [self sendPacket:data timeoutInterval:60.0];
}
- (void)sendPacket:(NSData *)data timeoutInterval:(NSTimeInterval)timeoutInterval {
    [socket writeData:data withTimeout:timeoutInterval tag:0];
    NSLog(@"remote %@ send>>>>>>>>>>>%@", socket, [self hex:data]);
}
